defmodule PulseWidthTestHelper do
  @moduledoc false
  ...
  def delete_all_devices do
    all_devices = PulseWidth.devices_begin_with("")

    for device_name <- all_devices do
      case PulseWidth.device_find(device_name) do
  ...
  def execute_cmap(%{cmd_map: %{name: _} = cmap} = ctx), do: %{ctx | execute_rc: PulseWidth.execute(cmap)}
  ...
  defp freshen(ctx) do
    ensure_current = fn %{name: name} = ctx ->
      for _x <- 1..10, reduce: PulseWidth.status(name) do
        %{ttl_expired: true} ->
          ["ttl expired after freshen: ", name, " (will retry)"] |> Logger.debug()
          Process.sleep(50)
          {ctx, PulseWidth.status(name)}
  ...
  def freshen_auto(%{device_actual: %Device{}, name: name} = ctx) do
    case PulseWidth.status(name) do
  ...
  def make_alias(%{make_alias: true, pio: pio, name: name, device_actual: device_actual} = ctx) do
    ...
    %{ctx | alias_create: PulseWidth.alias_create(device_name, name, pio, opts)}
  end
