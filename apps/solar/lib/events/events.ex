defmodule Solar.Events do
  ...
  defp get_local_mean_time(state) do
    local_mean_time =
      state[:sun_local_hour] + state[:right_ascension] -
  ...
  defp get_local_time(state) do
    ...
    result = Timex.local() |> Timex.set(hour: hour, minute: minute, second: seconds, microsecond: 0)

    {:ok, Map.put(state, :local_time, result)}
  end
