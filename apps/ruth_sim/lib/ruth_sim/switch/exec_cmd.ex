defmodule SwitchSim.ExecCmd do
  require Logger

  def inbound_cmd_msg(%{device: dev_name, host: msg_host, pio: pio} = cmd_msg) do
    alias SwitchSim.Report

    with %SwitchSim{host: h} = sim when msg_host == h <- SwitchKeeper.load(dev_name),
         {:exec_cmd, exec_cmd} <- get_exec_cmd(cmd_msg),
         {:valid_exec_cmd, exec_cmd} <- validate_exec_cmd(exec_cmd) do
      # 1. the SwitchSim has been found for the cmd msg
      # 2. retrieved the exec cmd
      # 3. validated the exec cmd
      # 4. hand-off to SwitchSim to apply the exec cmd, returns the updated SwitchSim
      SwitchSim.apply_exec_cmd(sim, pio, exec_cmd)
